...
const SignUp = () => {
  ...
  const nameChangeHandler = (
    changedName: React.ChangeEvent<HTMLInputElement>
  ) => {
    setName(changedName.target.value);
  };

  const surnameChangeHandler = (
    changedSurname: React.ChangeEvent<HTMLInputElement>
  ) => {
    setSurname(changedSurname.target.value);
  };

  const emailAddressChangeHandler = (
    changedEmailAddress: React.ChangeEvent<HTMLInputElement>
  ) => {
    setEmailAddress(changedEmailAddress.target.value);
  };

  const passwordChangeHandler = (
    changedPassword: React.ChangeEvent<HTMLInputElement>
  ) => {
  ...
  const onClickHandler = (event: React.ChangeEvent<{}>) => {
    ...
    axios
      .post(
        "http://localhost:1400/user/sign/signup",
        new SignUpRequestBody(name, surname, emailAddress, password)
      )
